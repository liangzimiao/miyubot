...
from utils import concat_pic, get_event_gid, pic2b64
...
@switch_pick.handle()
async def set_pool(bot, event: MessageEvent, state: T_State):
    gc_id = get_event_gid(event)
...
@gacha_one.handle()
async def gacha_1(bot, event: MessageEvent):
...
@gacha_ten.handle()
async def gacha_10(bot, event: MessageEvent):
...
async def gacha_300(bot, event: MessageEvent):
    gc_id = get_event_gid(event)
    gacha = Gacha(_group_pool[gc_id])
