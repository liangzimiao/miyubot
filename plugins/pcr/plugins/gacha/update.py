...
async def update_pcrdata():
    ...
    for id in online_pcrdata:
        ...
        if (id not in pcr_data.CHARA_NAME or len(pcr_data.CHARA_NAME[id]) == 1) and id != 9401:
            ...
            # todo  缓存数据，未写入文件
            pcr_data.pcr_data.add_chara(id, m)
...
def ids2names(ids: list) -> list:
    ...
    res = []
    for id in ids:
        if id in pcr_data.CHARA_NAME:
            res.append(pcr_data.CHARA_NAME[id][0])
        else:
            logger.warning(f'缺少角色{id}的信息, 请注意更新静态资源')
    return res
...
def get_local_ver() -> int:
    ...
    if os.path.exists(local_ver_path):
        ...
        local_ver = 0
    logger.info(f'检查卡池更新, 本地卡池版本{local_ver}')
    return local_ver
            ...
            if online_pool_name in pool_name[server]:
                ...
                for star in ids_list:
                    ...
                    if not local_pool[server][star]:
                        ...
                        logger.info(f'{server}卡池{star}列表为空, 已替换为镜华(万圣节)')

    # 将新卡池写入文件
    logger.info(f'开始写入本地卡池文件')
...
update = on_command('更新卡池',permission=SUPERUSER)


@update.handle()
async def update_pool_chat(bot: Bot, event: Event):
    """
    手动更新卡池时试用此命令
    """
    print(str(BotInfo.bot_id))

    #if event.get_user_id() != superUser:
    #    return
...
force_update = on_command('强制更新卡池',permission=SUPERUSER)


@force_update.handle()
async def update_pool_force_chat(bot: Bot, event: Event):
    """
    强制更新卡池
    """
    ...
    if status == 0:
        await force_update.finish(f'状态{status}')
    elif status < 1000:
        await force_update.finish(f'发生错误{status}')
    else:
        await force_update.finish(f'更新完成, 当前卡池版本{status}')
