...
from .img_exploration import Imgexploration
...
imgexploration = on_command(cmd="搜图", priority=1, block=False)
    ...
    for segment in msgpic:
        if segment.type == "image":
            ...
            await imgexploration.send(
                message=Message(
                    MessageSegment.image(file=result_dict["pic"]) + MessageSegment.text(f'total:{len(result_dict["info"])}\n请在180s内发送序号以获得对应结果的链接，一次可以发送多个序号，例如：1 5 6'),
                ),
                reply_message=True,
            )
            break
