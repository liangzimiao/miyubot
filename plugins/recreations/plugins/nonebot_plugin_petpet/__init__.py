...
from .data_source import memes,Petpet
...
help_cmd = Petpet().on_command("头像表情包","头像表情包", aliases={"头像相关表情包", "头像相关表情制作"}, block=True, priority=5)
block_cmd = Petpet().on_command("禁用表情", "禁用表情",block=True, priority=5, permission=PERM_EDIT)
unblock_cmd = Petpet().on_command("启用表情","启用表情", block=True, priority=5, permission=PERM_EDIT)
block_cmd_gl = Petpet().on_command("全局禁用表情","全局禁用表情", block=True, priority=5, permission=PERM_GLOBAL)
unblock_cmd_gl = Petpet().on_command("全局启用表情","全局启用表情", block=True, priority=5, permission=PERM_GLOBAL)
...
def create_matchers():
    ...
    Petpet().on_message(rule=regex("随机表情"), block=False, priority=5).append_handler(random_handler())
