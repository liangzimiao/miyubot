...
real_esrgan = RealESRGAN().on_command(
   "超分","超分", aliases={"real-esrgan", "超分辨率重建", "esrgan", "real_esrgan","重建","清晰术"}, priority=5
)
...
async def real_esrgan_handle_img(event: MessageEvent, state: T_State):
    ...
    seg = state["img"]
    if seg[0].type == "image":
        img = await get_img(seg[0].data["url"])
        await real_esrgan.send(Message("请稍后，正在处理中"))
    else:
        await real_esrgan.finish(Message(f"不是图捏"),at_sender=True)
    # 下面来处理图片
    try:
