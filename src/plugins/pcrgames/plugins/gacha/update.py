...
async def update_pcrdata():
    ...
    for id in online_pcrdata:
        ...
        if (id not in chara_master.CHARA_NAME or len(chara_master.CHARA_NAME[id]) == 1) and id != 9401:
            ...
            # todo  缓存数据，未写入文件
            chara_master.chara_master.add_chara(id, m)
...
def ids2names(ids: list) -> list:
    ...
    res = []
    for id in ids:
        if id in chara_master.CHARA_NAME:
            res.append(chara_master.CHARA_NAME[id][0])
        else:
...
force_update = on_command('强制更新卡池')
