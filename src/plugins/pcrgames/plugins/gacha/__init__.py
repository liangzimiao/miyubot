...
try:
    ...
    logger.warning('group_pool_config.json not found, will create when needed.')
...
@switch_pick.handle()
async def set_pool(bot, event: GuildMessageEvent, state: T_State):
    gc_id = f'{event.channel_id}_{event.guild_id}'
...
@gacha_one.handle()
async def gacha_1(bot, event: GuildMessageEvent):
...
@gacha_ten.handle()
async def gacha_10(bot, event: GuildMessageEvent):
    ...
    if hiishi >= SUPER_LUCKY_LINE:
        await gacha_ten.finish('恭喜海豹！おめでとうございます！')
    await gacha_ten.finish(Message(f'素敵な仲間が増えますよ！\n{res}\n'), at_sender=True)
...
async def gacha_300(bot, event: GuildMessageEvent):
    gc_id = f'{event.channel_id}_{event.guild_id}'
    gacha = Gacha(_group_pool[gc_id])
    ...
    await gacha_sanhyaku.finish(Message('\n'.join(msg)), at_sender=True)
