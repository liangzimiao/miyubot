            ...
            for file_number in file_number_list:
                ...
                if file_name.rsplit('.', 1)[0] not in file_name_list_no_suffix:
                    ...
                    if not await download(url, file_path):
                        ...
                        count = count + 1
        await bot.send(f'下载完毕，此次下载"cygames"语音包{count}个，目前共{len(os.listdir(DIR_PATH))}个. 如果您使用的是go-cqhttp，请更新到v0.9.28或更高的版本并自行配置ffmpeg，否则无法发送m4a格式的语音.')


matcher = on_command("猜语音排行", aliases={"猜语音排行", "猜语音排行榜", "猜语音群排行"},priority=5)
@matcher.handle()
...
matcher = on_command("cygames", priority=5)
@matcher.handle()
...
matcher = on_command("猜语音", priority=5)
@matcher.handle()
